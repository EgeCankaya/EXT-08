...
**Date:** 2026-08-30
**Revision:** 7 — OQ-4 resolved at M6; R7 reframed after the reference instrument turned out to be lossy. See §"Revision history".
**Status:** Draft
...
| **R8 — The host EXT-17 will actually use has never been checked for repeatability.** [S2] states the simulation is deterministic by contract and makes "two identical runs produce identical captures" a hard gate at its step 4 — *"do not build further until it passes"*. M4 measured ~1% of frames never published, run to run, but on **`n8ro-sim-local.exe`**, a wall-clock-paced test driver, not on the headless `n8ro-sim-app.exe` in the closed configuration a campaign uses | High for EXT-17 — its step 4 gate either passes or its whole self-test design changes; low for EXT-08, whose own guarantee is now scoped to the recorder | Unknown, and cheap to resolve | **Spike it: two identical runs on `n8ro-sim-app.exe`, captures compared byte-for-byte.** Blocked only by OQ-2 (the invocation). Half an hour once that is answered, and it decides EXT-17's self-test design before a line of it is written. Whichever way it goes, `docs/capture-format-v1.md` §14 records the result |
