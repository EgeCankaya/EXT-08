...
**Date:** 2026-08-31
**Revision:** 13 — audit against [S1] read directly off disk. **Every requirement and every deliverable is met, and all 41 source items are accounted for.** Four stale status rows corrected; one advisory gap closed by test. See §"Revision history".
...
- `--capture-max-samples` stops a run after N `sample` records **across the whole run** and closes with `end_reason: size_limit`. It is a **record-count safety bound, and a different bound from CAP-6's** — it counts records rather than bytes, it is run-wide rather than per file, and it always stops rather than offering the rotate choice. Both exist; neither absorbed the other. See `docs/decisions-m5-m7.md` D-13 and D-38.
...
- Drop counts are per-topic, so a loss can be attributed. *(Implemented per **kind** — sample versus structural — not per topic, with the two event topics merged under `events_not_recorded`. See `docs/decisions-m5-m7.md` D-7 for why, and `docs/escalations.md` E-8: either this criterion is amended to per-kind or the trailer gains per-topic keys, and that is EXT-17's call because it reads them.)*
