...
- Drop counts are per-topic, so a loss can be attributed. *(Implemented per **kind** — sample versus structural — not per topic, with the two event topics merged under `events_not_recorded`. See `docs/escalations.md` E-8: either this criterion is amended to per-kind or the trailer gains per-topic keys, and that is EXT-17's call because it reads them.)*
